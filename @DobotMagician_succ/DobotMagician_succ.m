classdef DobotMagician_succ < RobotBaseClass
    %% DobotMagician
...
%% Constructor 
        function self = DobotMagician_succ(baseTr)
			self.CreateModel();
            if nargin == 1			
...
%% CreateModel
        function CreateModel(self)
            ...
            link(5) = Link('d',-0.08,      'a',0,      'alpha',0,      'offset',pi, 'qlim',[deg2rad(-85),deg2rad(85)]);

            % end effector parameters
             self.useTool = 1;
             self.toolFilename = 'DobotMagician_succ_tool.ply';
             self.toolTr = transl(0, 0, 0);
